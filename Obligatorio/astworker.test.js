...
test('Fail at let', () => {
    expect(() => {
        ASTworker("let x = 1;", null);
    }).toThrow(/Let.*.*/);
});

test('Fail at comma operation', () => {
    expect(() => {
        ASTworker("var x = (1,2);", null);
    }).toThrow(/Comma.*/);
});

test('Fail at multi declaration', () => {
    expect(() => {
        ASTworker("var x = 1, y = 2;", null);
...
test('Replace by multiassign', () => {
    ...
    var x = [1,2,3,4];
    x[1,2,3] = 1;
    `, null).replace(/\s/g, '')).toMatch(`let x = [1, 2, 3, 4];
        x[3] = x[2] =x[1] =1;
    `.replace(/\s/g, ''));
});
...
test('Replace named function for const function', () => {
    ...
    `, null).replace(/\s/g, '')).toMatch(`const f = functionf(x){return x;}
    `.replace(/\s/g, ''));
});
